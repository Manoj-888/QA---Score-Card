...
// ─── Email Modal ──────────────────────────────────────────────────────────────
function EmailModal({ submission, form, fields, allQ, onClose }) {
  const [email, setEmail] = useState('')
  const [sending, setSending] = useState(false)
  const [sent, setSent] = useState(false)
  const [err, setErr] = useState('')

  const agentName = submission.meta?.employeeName || submission.meta?.employee_name || '—'
  const ticketId  = submission.meta?.ticketId    || submission.meta?.ticket_id    || '—'
  const manager   = submission.meta?.reportingManager || submission.meta?.reporting_manager || '—'
  const score     = submission.pct

  const buildEmailBody = () => {
    const lines = []
    lines.push('Quality Scorecard Report')
    lines.push('========================')
    lines.push('Form: ' + form.title)
    lines.push('Ticket ID: ' + ticketId)
    lines.push('Agent: ' + agentName)
    lines.push('Reporting Manager: ' + manager)
    lines.push('Date: ' + new Date(submission.created_at).toLocaleDateString('en-GB'))
    lines.push('')
    lines.push('SCORE: ' + submission.earned + ' / ' + submission.possible + ' pts (' + score + '%)')
    lines.push('')
    lines.push('Question Breakdown:')
    lines.push('-------------------')
    allQ.forEach(q => {
      const ans = (submission.answers?.[q.id] || 'N/A').toUpperCase()
      lines.push('[' + q.path + '] ' + q.text + ' (' + q.points + 'pts) → ' + ans)
    })
    lines.push('')
    lines.push('This report was generated by ScoreCard.')
    return lines.join('\n')
  }
  ...
  const handleSend = async () => {
    ...
    try {
      const subject = encodeURIComponent('Scorecard Report — ' + agentName + ' | Ticket ' + ticketId + ' | ' + score + '%')
      const body    = encodeURIComponent(buildEmailBody())
      window.open('mailto:' + email.trim() + '?subject=' + subject + '&body=' + body, '_blank')
      setSent(true)
    } catch(e) {
      setErr('Failed to open email client.')
    } finally {
      setSending(false)
    }
  }
        ...
        <div className="modal-body">
          {sent ? (
            <div style={{textAlign:'center',padding:'16px 0'}}>
              ...
              <p style={{fontWeight:600,marginBottom:6}}>Email client opened!</p>
              <p style={{color:'var(--text2)',fontSize:13}}>Your email app should have opened with the pre-filled report. Review and hit Send.</p>
              <button className="btn secondary" style={{marginTop:18,width:'100%'}} onClick={onClose}>Close</button>
            </div>
          ) : (
            <>
              <div className="email-preview-box">
                <div className="email-preview-row"><span className="email-preview-lbl">Agent</span><span>{agentName}</span></div>
                <div className="email-preview-row"><span className="email-preview-lbl">Ticket</span><span>{ticketId}</span></div>
                <div className="email-preview-row"><span className="email-preview-lbl">Score</span><span className={`score-pill ${score>=80?'green':score>=60?'yellow':'red'}`}>{score}%</span></div>
                <div className="email-preview-row"><span className="email-preview-lbl">Manager</span><span>{manager}</span></div>
              </div>
              <label className="meta-label" style={{marginTop:14}}>Send to Email Address</label>
              <input
                className={`meta-input ${err?'error':''}`}
                type="email"
                value={email}
              ...
              />
              {err && <div className="error-msg" style={{margin:'8px 0 0',padding:'8px 12px'}}>{err}</div>}
              <p style={{color:'var(--text3)',fontSize:12,marginTop:8}}>This will open your default email client with the scorecard report pre-filled.</p>
              <div style={{display:'flex',gap:10,marginTop:16}}>
                <button className="btn secondary" style={{flex:1}} onClick={onClose}>Cancel</button>
                <button className="btn primary" style={{flex:1}} onClick={handleSend} disabled={sending||!email.trim()}>
                  {sending?<><Spinner size={14}/> Opening…</>:'✉️ Send Report →'}
                </button>
              </div>
            </>
          )}
...
function PageReports({ submissions, form, fields, onDelete, loading }) {
  ...
  const managerField = fields.find(f=>f.key==='reportingManager'||f.label?.toLowerCase().includes('manager'))
  const agentField   = fields.find(f=>f.key==='employeeName'||f.label?.toLowerCase().includes('employee')||f.label?.toLowerCase().includes('agent'))
  const managerKey   = managerField?.key || 'reportingManager'
  const agentKey     = agentField?.key   || 'employeeName'
  ...
  filtered.forEach(sub=>{
    const mgr   = sub.meta?.[managerKey]?.trim() || '(No Manager)'
    const agent = sub.meta?.[agentKey]?.trim()   || '(Unknown Agent)'
    if(!grouped[mgr]) grouped[mgr]={}
    if(!grouped[mgr][agent]) grouped[mgr][agent]=[]
    grouped[mgr][agent].push(sub)
  })

  const toggleManager = mgr => setCollapsedManagers(p=>({...p,[mgr]:!p[mgr]}))
  const toggleAgent   = key => setCollapsedAgents(p=>({...p,[key]:!p[key]}))
  ...
  const SubTable = ({ subs }) => (
    <div className="report-table-wrap" style={{borderRadius:'0 0 8px 8px',border:'none',borderTop:'1px solid var(--border)'}}>
      <table className="report-table">
        <thead><tr>
          {fields.filter(f=>f.key!==managerKey).map(f=>(
            <th key={f.id} className="sortable" onClick={()=>toggleSort(f.key)}>
        ...
        <tbody>
          {subs.map(sub=>(
            <tr key={sub.id} className="report-row" onClick={()=>setSelected(sub)}>
              {fields.filter(f=>f.key!==managerKey).map(f=><td key={f.id}>{sub.meta?.[f.key]||'—'}</td>)}
              ...
              <td onClick={e=>e.stopPropagation()} style={{display:'flex',gap:6,alignItems:'center'}}>
                <button className="btn-sm secondary" title="Send report by email" onClick={()=>setEmailTarget(sub)} style={{padding:'4px 8px',fontSize:12}}>✉️</button>
  ...
  return (
    <div className="page-content">
      <div className="page-hero">
        ...
        <p className="page-sub">{submissions.length} total submissions — grouped by Reporting Manager</p>
      </div>
      ...
      <div className="report-toolbar">
        <input className="search-input" placeholder="🔍 Search by ticket, employee, manager…" value={searchStr} onChange={e=>setSearchStr(e.target.value)} />
      ...
      {!loading&&submissions.length===0?(
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          {Object.entries(grouped).map(([mgr,agents])=>{
            const mgrSubs=Object.values(agents).flat()
            const mgrPcts=mgrSubs.map(s=>s.pct)
            const mgrAvg=Math.round(mgrPcts.reduce((a,b)=>a+b,0)/mgrPcts.length)
            const isCollapsed=collapsedManagers[mgr]
            return (
              <div key={mgr} className="manager-group">
                <div className="manager-header" onClick={()=>toggleManager(mgr)}>
                  <div style={{display:'flex',alignItems:'center',gap:12}}>
                    <span className="group-chevron">{isCollapsed?'▶':'▼'}</span>
                    <span>👤</span>
                    <span className="manager-name">{mgr}</span>
                    <span className="group-badge">{Object.keys(agents).length} agent{Object.keys(agents).length!==1?'s':''}</span>
                    <span className="group-badge">{mgrSubs.length} ticket{mgrSubs.length!==1?'s':''}</span>
                  </div>
                  <span className={`score-pill ${mgrAvg>=80?'green':mgrAvg>=60?'yellow':'red'}`} style={{marginRight:8}}>Avg {mgrAvg}%</span>
                </div>
                {!isCollapsed&&(
                  <div className="manager-body">
                    {Object.entries(agents).map(([agent,subs])=>{
                      const agentGroupKey=mgr+'::'+agent
                      const agentPcts=subs.map(s=>s.pct)
                      const agentAvg=Math.round(agentPcts.reduce((a,b)=>a+b,0)/agentPcts.length)
                      const isAgentCollapsed=collapsedAgents[agentGroupKey]
                      return (
                        <div key={agent} className="agent-group">
                          <div className="agent-header" onClick={e=>{e.stopPropagation();toggleAgent(agentGroupKey)}}>
                            <div style={{display:'flex',alignItems:'center',gap:10}}>
                              <span className="group-chevron">{isAgentCollapsed?'▶':'▼'}</span>
                              <span>🧑‍💼</span>
                              <span className="agent-name">{agent}</span>
                              <span className="group-badge">{subs.length} ticket{subs.length!==1?'s':''}</span>
                            </div>
                            <span className={`score-pill ${agentAvg>=80?'green':agentAvg>=60?'yellow':'red'}`}>Avg {agentAvg}%</span>
                          </div>
                          {!isAgentCollapsed&&<SubTable subs={subs}/>}
                        </div>
                      )
                    })}
                  </div>
          ...
          <div className="modal-card" onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              ...
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <button className="btn-sm secondary" onClick={()=>{setEmailTarget(selected);setSelected(null)}}>✉️ Send Report</button>
      ...
      {emailTarget&&(
        <EmailModal submission={emailTarget} form={form} fields={fields} allQ={allQ} onClose={()=>setEmailTarget(null)}/>
      )}
    </div>
  )
}
...
.manager-group{background:var(--card);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden}
.manager-header{display:flex;justify-content:space-between;align-items:center;padding:14px 18px;background:var(--card2);cursor:pointer;user-select:none;border-bottom:1px solid transparent;transition:background .15s}
.manager-header:hover{background:var(--bg3)}
.manager-body{padding:12px;display:flex;flex-direction:column;gap:10px}
.manager-name{font-weight:600;font-size:14px}
.agent-group{background:var(--bg3);border:1px solid var(--border);border-radius:8px;overflow:hidden}
.agent-header{display:flex;justify-content:space-between;align-items:center;padding:11px 14px;cursor:pointer;user-select:none;transition:background .15s}
.agent-header:hover{background:var(--bg2)}
.agent-name{font-weight:500;font-size:13.5px}
.group-chevron{font-size:11px;color:var(--text3);width:14px;display:inline-block}
.group-badge{background:var(--bg2);border:1px solid var(--border);padding:2px 8px;border-radius:99px;font-size:11px;color:var(--text2);font-weight:500}
.email-preview-box{background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:12px 14px;display:flex;flex-direction:column;gap:8px}
.email-preview-row{display:flex;justify-content:space-between;align-items:center;font-size:13px}
.email-preview-lbl{color:var(--text2);font-weight:600;font-size:11.5px;text-transform:uppercase;letter-spacing:.4px}
